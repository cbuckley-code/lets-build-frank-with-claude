...
//   /              -> the built console, when there is one
...
export function createApp({ config, tools = createTools(config) }: AppOptions): Express {
  ...
  app.get('/healthz', (_req, res) => {
    res.status(200).json({
      ...
      uptimeSeconds: Math.max(0, Math.floor((Date.now() - config.startedAt.getTime()) / 1000)),
    });
  });

  // Stateless Streamable HTTP: a fresh server and transport per request. Frank
  // scales to zero with one replica, so there is no session worth keeping — and
  // a session would break the moment he sleeps.
  ...
  // A browser pointed at /mcp gets something legible instead of a hang.
  ...
  // The console is a static build served from <package root>/public. It is
  // OPTIONAL (ADR-003 builds it late): Frank must serve MCP with ui/ empty, so
  // say so at / rather than letting static middleware fall through to a bare 404.
  ...
  if (existsSync(indexHtml)) {
    ...
    app.get('/', (_req, res) => {
      res
        ...
        .type('text/plain')
        .send(
          'The console has not been built yet (ADR-003). Frank is listening at POST /mcp and GET /healthz.',
        );
    });
  }
